    ...
    <div className="space-y-6">
      <header>
        ...
        <p className="mt-2 max-w-3xl text-secondary">
          Tabs jump through the plan; cards expose daily execution, transitions, and the mistakes to avoid each week.
        </p>
      </header>
          ...
          {ROADMAP_WEEKS.map((week) => (
            <button
              ...
              className={`flex items-center gap-1 border-b-2 px-3 py-2 text-sm transition-colors ${
                currentWeek === week.week ? "border-accent text-accent" : "border-transparent text-secondary hover:text-primary"
              }`}
            >
      ...
      <div className="grid gap-4 xl:grid-cols-2">
        {ROADMAP_WEEKS.map((week) => {
          const hasReasoningTodo = week.transitionReasoning.includes("[TODO");
          const hasMistakesTodo = week.commonMistakes.includes("[TODO");
          const hasAnyTodo = hasReasoningTodo || hasMistakesTodo;

          return (
            <article
              id={`week-${week.week}`}
              key={week.week}
              className={`scroll-mt-32 rounded-card border bg-surface p-5 transition-all ${
                hasAnyTodo ? "border-amber-500/40 shadow-lg shadow-amber-500/5" : "border-border"
              }`}
            >
              <div className="flex flex-wrap items-start justify-between gap-3">
                <div>
                  <div className="flex items-center gap-2">
                    ...
                    {hasAnyTodo && (
                      <span className="inline-flex items-center gap-1.5 rounded-full bg-amber-500/20 border border-amber-500/50 px-2.5 py-0.5 text-2xs font-mono font-bold text-amber-400 animate-pulse">
                ...
                <button
                  ...
                  className="rounded-card border border-accent/40 bg-accent-dim px-3 py-2 text-sm text-accent hover:bg-elevated"
                >
                  Start This Week
                </button>
              </div>
              ...
              <blockquote className="mt-5 border-l-2 border-accent bg-accent-dim/30 px-4 py-3 text-sm text-secondary">
                {week.weeklyGoal}
              </blockquote>
              
              {/* Transition Reasoning Section */}
              <div className={`mt-4 rounded-card border p-3 text-sm transition-all ${
                hasReasoningTodo
                ...
                {hasReasoningTodo && (
                  <div className="mb-2 flex items-center gap-2">
                    <span className="inline-flex items-center gap-1.5 rounded-md bg-amber-500/20 border border-amber-500/40 px-2 py-0.5 text-2xs font-mono font-bold text-amber-300 animate-pulse">
                      <span className="h-1.5 w-1.5 rounded-full bg-amber-400 animate-ping" />
                      ⚠️ BLINKING TODO: COMPLETE TRANSITION REASONING
                    </span>
                  </div>
                )}
              ...
              <div className={`mt-3 rounded-card border p-3 text-sm transition-all ${
                hasMistakesTodo
                ...
                {hasMistakesTodo && (
                  <div className="mb-2 flex items-center gap-2">
                    <span className="inline-flex items-center gap-1.5 rounded-md bg-rose-500/20 border border-rose-500/40 px-2 py-0.5 text-2xs font-mono font-bold text-rose-300 animate-pulse">
                      <span className="h-1.5 w-1.5 rounded-full bg-rose-400 animate-ping" />
                      ⚠️ BLINKING TODO: COMPLETE COMMON MISTAKES
                    </span>
                  </div>
                )}
